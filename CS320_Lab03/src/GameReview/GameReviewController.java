...
public class GameReviewController {
	...
	public Integer deleteCredentialsIntoCredentialsTable(String username, String password) {
		return model.getDb().deleteCredentialsIntoCredentialTable(username, password);
	}
	public boolean doesUsernameAlreadyExist(String username) {
		...
		for (int i = 0; i<credentialList.size(); i++) {
			if (username.equals(credentialList.get(i).getUsername())) {
				return true;
			}
		}
		return false;
	...
	public boolean validateCredentials(String pw, String un) {
		if (model.getDb().findCredentialsByUsername(un).isEmpty()) {
			return false;
		}
